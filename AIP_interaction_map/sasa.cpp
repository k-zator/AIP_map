...
static void asa_frame(const float* frame, const int n_atoms, const float* atom_radii,
                      ...
                      int* neighbor_indices, float* centered_sphere_points, float* areas, float* sphxyz)
{
    ...
    for (int i = 0; i < n_atoms; i++) {
        ...
        for (int j = 0; j < n_sphere_points; j++) {
            ...
            if (is_accessible) {
                sphxyz[3*j+3*n_sphere_points*i] = r_j[0];
                sphxyz[3*j+1+3*n_sphere_points*i] = r_j[1];
                sphxyz[3*j+2+3*n_sphere_points*i] = r_j[2];
                areas[i]++;
            }
        }
...
void sasa(const int n_frames, const int n_atoms, const float* xyzlist,
          ...
          const int* atom_mapping, const int n_groups, float* out, float* sphxyz)
{
  /*
  ...
  */
  ...
  for (i = 0; i < n_frames; i++) {
    asa_frame(xyzlist + i*n_atoms*3, n_atoms, atom_radii, sphere_points,
	      n_sphere_points, wb1, wb2, outframebuffer, sphxyz);
